...
void TrigScint_SlowControlGenerator::genFrame(uint32_t size) {
  ...
  std::cout << "[ TrigScint_SlowControlGenerator ]: Header [16:0]: "
  ...
  std::cout << "[ TrigScint_SlowControlGenerator ]: Header [31:17]: "
            << std::bitset<16>(header) << std::endl;
  toFrame(it, 2, &header);
  ...
  for (auto &subpacket : subpackets) {
    std::cout << "[ TrigScint_SlowControlGenerator ]: ROC subpacket j = " << j << " : "
  ...
  uint32_t tail{0};
  std::cout << "[ TrigScint_SlowControlGenerator ]: Tail: " << std::bitset<16>(tail)
            << std::endl;
  toFrame(it, 2, &tail);
...
std::vector<uint16_t> TrigScint_SlowControlGenerator::buildQIESubpackets(int qie_count) {
