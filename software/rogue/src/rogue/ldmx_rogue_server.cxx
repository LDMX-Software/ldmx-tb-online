...
int main(int argc, char **argv) {
  ...
  // Flag indicating if an Trigger Scintillator generator/receiver should be
  // used
  ...
  if (hcal)
    generator = emulators::HCalGenerator::create();
  else if (trig)
    generator = emulators::TrigScintGenerator::create();
  ...
  while (true) {
    if (tcp_command->startRun()) {
      ...
      std::cout << "[ LDMX ] Configuring run!" << std::endl;
    }
    tcp_command->resetStates(); 
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(int((1 / rate) * 1000)));
  }
  return EXIT_SUCCESS;
}
