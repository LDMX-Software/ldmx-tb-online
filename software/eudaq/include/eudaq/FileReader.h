...
class FileReader {
 public:
   ...
   * @param[in] file_name full path to the file we are going to open
   */
  FileReader(const std::string& file_name) : FileReader() {
    this->open(file_name);
  }
   ...
   */
  template <typename WordType, std::enable_if_t<std::is_integral<WordType>::value,bool> = true>
  FileReader& read(WordType* w, std::size_t count) {
   ...
   */
  template <typename WordType, std::enable_if_t<std::is_integral<WordType>::value,bool> = true>
  FileReader& operator>>(WordType& w) {
    return read(&w, 1);
  }
  ...
  FileReader& read(std::vector<ContentType>& vec, std::size_t count) {
    vec.resize(count);
