...
std::optional<reformat::EventPacket> TrigScintEventGroupedRawFile::next() {
  ...
  static const std::size_t TIMESINCESPILL_LEN_BYTES = 3;
  uint64_t timeSpill=0;
  for (int iW = 0; iW <= TIMESINCESPILL_LEN_BYTES; iW++) {
    //Convert from little endian to big
    int pos = TIMESINCESPILL_POS + TIMESINCESPILL_LEN_BYTES - iW;
  ...
  if (timeSpill < last_event_time && timeSpill != 0) {
    i_spill_++;
    reformat_log(debug) << "new spill " << i_spill_;
  }
  last_event_time = timeSpill;

  uint64_t ts{(((uint64_t)i_spill_) << 32)+timeSpill};
